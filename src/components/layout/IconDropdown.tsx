...
import type { HeaderDropdownProps } from "@/types/header";
...
export function IconDropdown({
  ...
  if (loading)
    return <Skeleton className="w-5 h-5 md:w-10 md:h-10 rounded-full bg-background/20" />;
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <Avatar className="w-5 h-5 md:w-8 md:h-8">
            <AvatarImage src={selected?.icon || fallbackIcon} className="bg-white" />
