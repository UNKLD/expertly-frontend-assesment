      ...
      return events.map(
        (event): NormalizedMatch => ({
          ...
          homeTeam: {
            ...
            shortName: event.strHomeTeam?.substring(0, 3).toUpperCase() || "TBD",
          },
          awayTeam: {
            id: event.idAwayTeam,
            name: event.strAwayTeam,
            logo: event.strAwayTeamBadge || "",
            shortName: event.strAwayTeam?.substring(0, 3).toUpperCase() || "TBD",
          },
    ...
    async (isInitial = true) => {
      ...
      while (attemptsRef.current <= MAX_RETRIES && mountedRef.current) {
        try {
          ...
          const backoff = INITIAL_BACKOFF_MS * Math.pow(2, attemptsRef.current - 1);
